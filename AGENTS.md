...
- The configuration pages (`#/setting/base-assistant` and `#/setting/advanced-assistant`) populate the assistant picker by calling LnkChatBI's login-free Origin-bound endpoint `GET /api/v1/system/assistant/embed/list?type=0|1` **directly from the browser**. The browser auto-attaches the `Origin` header; LnkChatBI filters by each assistant's configured cross-domain whitelist and returns `{ code: 0, data: [{id, name, type}] }`. No `access_token`, no demo-backend proxy. The `access_token` column in the `setting` table is retained for rollback safety but no longer read or written.
